...
class BaseCausalInference:
    def __init__(self, ts_list, maxlags=3, ground_truth=None, n_jobs=1, suffix="", manages_own_parallelism=False):
        ...
        self.manages_own_parallelism = manages_own_parallelism
    ...
    def run(self):
        ...
        if self.n_jobs == 1 or self.manages_own_parallelism:
            print('Running in single-threaded mode or managing parallelism manually.')
            for ts_index, ts in tqdm(ts_tuples, desc="Processing Time Series"):
                _, causal_df = self.process_ts((ts_index, ts))
                self.causal_dfs[ts_index] = causal_df
        else:
            print(f"Running parallel inference over {len(ts_tuples)} time series using {self.n_jobs} jobs...")
            # We need this to avoid pickling a self attribute WHILE multiprocessing
            results_list = []
            with Pool(processes=self.n_jobs) as pool:
                results_iterator = pool.imap_unordered(self.process_ts, ts_tuples)
                
                # Wrap with tqdm for a single, clean progress bar
                for result in tqdm(results_iterator, total=len(ts_tuples), desc="Processing Time Series"):
                     results_list.append(result)
            
            for ts_index, causal_df in results_list:
                self.causal_dfs[ts_index] = causal_df
        return self
    ...
    def build_causeme_matrices(self, n_variables):
        ...
        for ts_index, causal_df in self.causal_dfs.items():
            ...
            for source_variable_index in range(n_variables):
                # filter by source
                source_df = causal_df_2.loc[
                    causal_df_2["source"] % n_variables == source_variable_index
                ]
                for target_variable_index in range(n_variables):
                    # filter by target
                    target_df = source_df.loc[
                        source_df["target"] == target_variable_index
                    ].reset_index(drop=True)

                    # make 'pvalue' column numeric
                    target_df["pvalue"] = pd.to_numeric(
                        target_df["pvalue"], errors="coerce"
                    )
                    ...
                    if lowest_p_value_row["is_causal"] == 1:
                        self.causal_matrices[ts_index][
                            source_variable_index, target_variable_index
                        ] = lowest_p_value_row["value"]
                        self.p_value_matrices[ts_index][
                            source_variable_index, target_variable_index
                        ] = lowest_p_value_row["pvalue"]
                        self.lag_matrices[ts_index][
                            source_variable_index, target_variable_index
                        ] = lag
        ...
        for ts_idx in range(len(self.ts_dict)):
            ...
            metrics.append(
                [
                    method_name,
                    "precision",
                    precision_score(y_test, y_hat, zero_division=np.nan),
                ]
            )
            metrics.append(
                [
                    method_name,
                    "recall",
                    recall_score(y_test, y_hat, zero_division=np.nan),
                ]
            )
            metrics.append(
                [method_name, "f1", f1_score(y_test, y_hat, zero_division=np.nan)]
            )
            metrics.append(
                [
                    method_name,
                    "balanced_error",
                    1 - balanced_accuracy_score(y_test, y_hat),
                ]
            )
