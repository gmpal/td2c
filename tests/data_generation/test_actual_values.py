...
def test_observations_from_model_1(initialization):
    ...
    ts_builder = TSBuilder(
        observations_per_time_series=observations_per_time_series,
        n_variables=n_variables,
        time_series_per_process=time_series_per_process,
        processes_to_use=[model],
        max_neighborhood_size=max_neighborhood_size,
        seed=seed,
        noise_std=0,
    )
    ts_builder.build()
    neighbors = ts_builder.get_generated_neighbors()
            ...
            for row_idx, row in enumerate(data):
                if row_idx > starting_time:
                    for variable_idx, value in enumerate(row):
                        ...
                        Y_bar_t = np.mean(
                            observations[model_id][ts_index][row_idx - 1][
                                variable_neighbors
                            ]
                        )
                        Y_bar_t_minus_1 = np.mean(
                            observations[model_id][ts_index][row_idx - 2][
                                variable_neighbors
                            ]
                        )
                        assert value == -0.4 * (3 - Y_bar_t**2) / (
                            1 + Y_bar_t**2
                        ) + 0.6 * (3 - (Y_bar_t_minus_1 - 0.5) ** 3) / (
                            1 + (Y_bar_t_minus_1 - 0.5) ** 4
                        )
            ...
            for row_idx, row in enumerate(data):
                if row_idx > starting_time:
                    for variable_idx, value in enumerate(row):
                        ...
                        Y_bar_t_minus_2 = np.mean(
                            observations[model_id][ts_index][row_idx - 3][
                                variable_neighbors
                            ]
                        )
                        assert (
                            value
                            == (0.4 - 2 * math.exp(-50 * Y_bar_t_minus_1**2))
                            * Y_bar_t_minus_1
                            + (0.5 - 0.5 * math.exp(-50 * Y_bar_t_minus_2**2))
                            * Y_bar_t_minus_2
                        )
            ...
            for row_idx, row in enumerate(data):
                if row_idx > starting_time:
                    for variable_idx, value in enumerate(row):
                        ...
                        assert value == 1.5 * math.sin(
                            math.pi / 2 * Y_bar_t_minus_1
                        ) - math.sin(math.pi / 2 * Y_bar_t_minus_2)
            ...
            for row_idx, row in enumerate(data):
                if row_idx > starting_time:
                    for variable_idx, value in enumerate(row):
                        ...
                        assert (
                            value
                            == 2 * math.exp(-0.1 * Y_bar_t**2) * Y_bar_t
                            - math.exp(-0.1 * Y_bar_t_minus_1**2) * Y_bar_t_minus_1
                        )
            ...
            for row_idx, row in enumerate(data):
                if row_idx > starting_time:
                    for variable_idx, value in enumerate(row):
                        ...
                        assert value == 0.8 * math.log(
                            1 + 3 * Y_bar_t**2
                        ) - 0.6 * math.log(1 + 3 * Y_bar_t_minus_2**2)
                    ...
                    for variable_idx, value in enumerate(row):
                        ...
                        assert (
                            ...
                            == (
                                0.4
                                - 2
                                * math.cos(40 * Y_bar_t_minus_2)
                                * math.exp(-30 * Y_bar_t_minus_2**2)
                            )
                            * Y_bar_t_minus_2
                            + (0.5 - 0.5 * math.exp(-50 * Y_bar_t_minus_1**2))
                            * Y_bar_t_minus_1
                        )
            ...
            for row_idx, row in enumerate(data):
                if row_idx > starting_time:
                    for variable_idx, value in enumerate(row):
                        ...
                        assert (
                            value
                            == (0.5 - 1.1 * math.exp(-50 * Y_bar_t**2)) * Y_bar_t
                            + (0.3 - 0.5 * math.exp(-50 * Y_bar_t_minus_2**2))
                            * Y_bar_t_minus_2
                        )
            ...
            for row_idx, row in enumerate(data):
                if row_idx > starting_time:
                    for variable_idx, value in enumerate(row):
                        ...
                        Y_bar_t_minus_3 = np.mean(
                            observations[model_id][ts_index][row_idx - 4][
                                variable_neighbors
                            ]
                        )
